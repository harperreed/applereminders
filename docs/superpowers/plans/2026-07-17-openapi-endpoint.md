...
Expected: the unauthenticated request fails with status 401 rather than 200
because bearer middleware still applies globally before the route is added.
...
Expected: 2 tests pass with zero warnings after separating direct document
validation from the route/TCC integration check.
...
Expected: all 242 tests pass in 37 suites, build output has zero warnings, shell syntax passes, and `git diff --check` prints nothing.
